...
impl RssData {
    ...
    pub async fn run_checks(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for job in self.cfg.jenkins_jobs.clone() {
            let res = client.post(job.as_str())
            ...
            match feed {
                Ok(feed) => {
                    ...
                    let date = feed.updated.unwrap_or(DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp(0, 0), Utc)).timestamp();

                    match self.update_times.get(&job) {
                        Some(val) if &date <= val => {
                        }
                        _ => {
                            let body = format!("Update: {}", feed.updated.unwrap_or(DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp(0, 0), Utc)));
                            Notification::new()
